...
func main() {
	...
	for {
		...
		case <-ticker.C:
			rows, err := db.Query(`
				SELECT id, title, project_id, status, created_at, duration_hours 
				FROM tasks 
				WHERE status != 'done' AND duration_hours > 0
			`)
			if err != nil {
			...
			for rows.Next() {
				var t taskForCheck
				rows.Scan(&t.ID, &t.Title, &t.ProjectID, &t.Status, &t.CreatedAt, &t.DurHours)
